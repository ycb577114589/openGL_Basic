...
INT WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd)
{
	...
	while (true)
	{
		...
		glScalef(1.0f,1.0f, 0.5f);//缩放，》1 变大， <1缩小
		glRotatef(30.0f, 0.0f, 0.0f, 1.0f);//旋转方向判断：  右手握拳，大拇指指向旋转轴方向， 四指弯曲方向，即为旋转方向
		glTranslated(1.0f, -1.0f, 0.0f);//移动方向
		
		glBegin(GL_TRIANGLES);//start to draw something
 // 
		glColor4ub(255, 0, 0, 255);//每个颜色占8字节  set color: white	
		glVertex3f(0.0f, 1.0f, -10.0f);

		glColor4ub(0, 0, 255, 255);//每个颜色占8字节  set color: white	
		glVertex3f(-1.0, 0.0f, -10.0f);

		glColor4ub(0, 255, 0, 255);//每个颜色占8字节  set color: white	
